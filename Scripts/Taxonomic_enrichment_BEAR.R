...
newdir<-file.path(getwd(), "Output/Taxonomic_enrichment_BEAR")
dir.create(newdir)
...
OrthoDB7_OGs <- levels(droplevels(OrthoDB7_Metazoa[grep("Nasonia vitripennis",OrthoDB7_Metazoa$Organism),"ODB7_OG_ID"]))
OrthoDB7_Metazoa <- OrthoDB7_Metazoa[which(OrthoDB7_Metazoa$ODB7_OG_ID%in%OrthoDB7_OGs),c("ODB7_OG_ID","Gene_ID","Organism","UniProt_Species")]

# Attach ODB code
OrthoDB7_SPECIES <- read.delim("./Input/OrthoDB7_SPECIES_tabtext")
OrthoDB7_SPECIES <- OrthoDB7_SPECIES[,c("ODB_code","Organism")]
OrthoDB7_Metazoa <- merge(OrthoDB7_Metazoa, OrthoDB7_SPECIES, by.x="Organism", by.y="Organism", all.x=T, all.y=F)

# Create list of species per OG
OG_species_list<-dlply(OrthoDB7_Metazoa, .(ODB7_OG_ID), .fun = summarize, ODB_code=levels(droplevels(ODB_code)))
...
OGS2_Taxascan<-OrthoDB7_Metazoa[which(OrthoDB7_Metazoa$ODB_code=="NVITR"),c("Gene_ID","ODB7_OG_ID")]
OGS2_Taxascan<-merge(OGS2_Taxascan, Taxascan, all.x=T, all.y=F)

# And save as csv
write.table(OGS2_Taxascan, file=file.path(newdir, "OGS2_Taxascan.csv"))
...
# Select only Nasonia genes
OrthoDB7_Nasonia<-OrthoDB7_Arthropoda[which(OrthoDB7_Arthropoda$Organism=="Nasonia vitripennis"),]

# For every Nasonia gene, extract highest and lowest level