...
newdir<-file.path(getwd(), "Output/sex_biased_nodes_CCRE")
dir.create(newdir)
graphdir<-file.path(getwd(), "Graphics/sex_biased_nodes_CCRE")
dir.create(graphdir)

# load datasets
eigenexon_evalues <- read.csv("./Output/CCREs/CCRE_nasdevgeneexon.csv")
names(eigenexon_evalues)[1] <- "eigenexonID"
# sexspec<- read.csv("./Output/sex_specific_nodes/sexspec_allstages.csv")[,-1]
# Remove sex-specific nodes
# eigenexon_evalues<-eigenexon_evalues[which((eigenexon_evalues$eigenexonID%in%sexspec[is.na(sexspec$spec)==F,"eigenexonID"])==F),]
...
# Save table of nodes with compressed stagebias
stagebiased_nodes_short <- apply(glms2_fdr_coef[,2:6], MARGIN = c(1,2), function(x){x<threshold})*apply(glms2_fdr_coef[,12:16], MARGIN = c(1,2), sign)
stagebiased_nodes_short <- apply(X = stagebiased_nodes_short, MARGIN = c(1,2), function(x){
  if (x==0) {"."} else if (x>0) {"+"} else if (x<0) {"-"}
})
stagebiased_nodes_short <- apply(X = stagebiased_nodes_short, 1, paste, collapse="")
stagebiased_nodes_short <- data.frame(glms2_fdr_coef$node_ID, stagebias = stagebiased_nodes_short)
sum(grepl("+|-", stagebiased_nodes_short$stagebias))
write.csv(x = stagebiased_nodes_short, file = file.path(newdir, "stagebiased_nodes_short_5E-2.csv"))
