...
newdir<-file.path(getwd(), "Output/stage_modularity_biweight_pseudopaired")
dir.create(newdir)
...
# Create lookup table for female samples to male pseudopaired replicates v1
lookup<-list()
lookup[[1]]<-data.frame(femaleSample=grep("female", names(nasdevgeneexon), value=T), maleSample=grep("female", names(nasdevgeneexon), value=T, invert = T))
lookup[[2]]<-data.frame(femaleSample=grep("female", names(nasdevgeneexon), value=T), maleSample=grep("female", names(nasdevgeneexon), value=T, invert = T))
lookup[[2]][,2]<-str_replace(lookup[[2]][,2], pattern = "[[:digit:]]$", replacement = c(3,1,2))
lookup[[3]]<-data.frame(femaleSample=grep("female", names(nasdevgeneexon), value=T), maleSample=grep("female", names(nasdevgeneexon), value=T, invert = T))
lookup[[3]][,2]<-str_replace(lookup[[3]][,2], pattern = "[[:digit:]]$", replacement = c(2,3,1))

# Create all possible female combinations
combinationsF<-t(combn(grep("female", names(nasdevgeneexon), value = T),3))
# Fetch matched male samples
combinationsM<-list()
combinationsM<-lapply(X = lookup,FUN = function(y){matrix(y[apply(combinationsF,c(1,2),function(x){grep(x,y[,1])}),2], ncol=3)})

# Convert to respective column numbers on data matrix
combinations<-lapply(combinationsM, function(x){cbind(combinationsF,x)})


# add sexbias information
combinations<-lapply(X = combinations, FUN = function(y){cbind(y,apply(y,1,function(x){sum(grepl("female",x))}))})
...
combinations<-lapply(X = combinations, FUN = function(y){
  cbind(y,adply(y,1,function(z){
...
# for every dataset remove column 8 (index generated by aaply) and name columns appropriately
combinations<-lapply(X = combinations, FUN = function(y){y[,-8]})
combinations<-lapply(X = combinations, FUN = function(y){
  names(y)[7:12]<-as.character(c("sexbias",levels(stage)))
  return(y)
  })

#### list version ends here
names(combinations)<-c("p1","p2","p3")
combinations<-ldply(combinations)

# convert samples removed to character and store in single column
combinations[sapply(combinations, is.factor)]<-sapply(combinations[sapply(combinations, is.factor)], as.character)
combinations$samplesremoved<-apply(X = combinations[,2:7],MARGIN = 1,FUN = function(x){paste0(x, collapse = ":")})
# then grep each sample to remove against names(nasdevgeneexon) to get indices in data.frame
combinations[,2:7]<-apply(combinations[,2:7],c(1,2), function(x){grep(x, names(nasdevgeneexon))})
names(combinations)<-c("pairing_id",paste(rep("x",6), 1:6, sep=""), names(combinations[,-c(1:7)]))
...
modularconnectivities<-alply(as.matrix(combinations[,2:7]), 1, function(expr){
  print(expr)
...
# save basic output
write.csv(modularconnectivities,file=file.path(newdir,"modularconnectivities_basic.csv"))
