...
newdir<-file.path(getwd(), "Output/sex_modularity_biweight")
dir.create(newdir)
graphdir<-file.path(getwd(), "Graphics/sex_modularity_biweight")
dir.create(graphdir)
...
powers<-read.csv(file="./Output/WGCNA_clustering_biweight_singleblock/WGCNA_biweight_power_selection.csv")
...
# Save
write.csv(modularconnectivities, file=file.path(newdir, "modularconnectivities_short.csv"))
...
kMain<-ddply(modularconnectivities, .(X1), numcolwise(sum))[,c("X1","kTotal")]
names(kMain)<-c("X1", "kMain")
modularconnectivities<-merge(modularconnectivities, kMain, by.x="X1", by.y="X1", all.x=T)

# add cluster size
transcriptsWithin<-ddply(modularconnectivities, .(clusterID, X1), nrow)
names(transcriptsWithin)<-c("clusterID", "X1", "clusterSize")
transcriptsWithin<-transcriptsWithin[unique(transcriptsWithin$clusterID),c("clusterID","clusterSize")]
...
# summarize by module
modulardensities<-ddply(modularconnectivities, .(clusterID, stage, sexbias, X1), numcolwise(median), na.rm = T)

# Save as csv
write.csv(modulardensities, file.path(newdir, "modulardensities_medians.csv"))
...
pdf(file = file.path(graphdir, "modularity_plots_mediandensities.pdf"), paper = "a4")

ggplot(modulardensities, aes(x=sexbias, y=dWithin, col=stage))+geom_point()+theme_bw()+geom_smooth(method="lm")+facet_wrap(~clusterID)+scale_color_brewer(type = "seq", palette = 4)+ggtitle("Integration vs Number of Female Samples Removed")
ggplot(modulardensities, aes(x=sexbias, y=dOut, col=stage))+geom_point()+theme_bw()+geom_smooth(method="lm")+facet_wrap(~clusterID)+scale_color_brewer(type = "seq", palette = 4)+ggtitle("Pleiotropy vs Number of Female Samples Removed")
ggplot(modulardensities, aes(x=dWithin, y=dOut, col=stage))+geom_point()+theme_bw()+geom_smooth(method="lm")+facet_wrap(~clusterID)+scale_color_brewer(type = "seq", palette = 4)+scale_x_log10()+scale_y_log10()+ggtitle("Integration vs Pleiotropy")

dev.off()


#### Modulewise version (sum within and outside cluster connectivities, then divide by maximum pairwise interactions possible)
